...
main(int argc, char* argv[])
{
    ...
    // directory. Rather than store them we generate them here programatically:
    if (IBTK_MPI::getRank() == 0)
    {
        std::ofstream output("cylinder2d.vertex");
        // a MATLAB version is available in examples/ConstraintIB/oscillating_rigid_cylinder/Cylinder2d.m

        const double dx = 32.0 / (64 * 4 * 4 * 4);
        const double dy = 16.0 / (32 * 4 * 4 * 4);

        const double x_com = 0.0;
        const double y_com = 0.0;
        const double radius = 0.5;

        const int num_pts_x = std::ceil(2 * radius / dx);
        const int num_pts_y = std::ceil(2 * radius / dy);
        ...
        const double mean_y = std::accumulate(ys.begin(), ys.end(), 0.0) / xs.size();

        for (double& x : xs) x -= mean_x;

        for (double& y : ys) y -= mean_y;

        output << xs.size() << '\n';
        output << std::fixed << std::setprecision(6);
        for (std::size_t i = 0; i < xs.size(); ++i) output << xs[i] << "\t" << ys[i] << '\n';
    }
    IBTK_MPI::barrier();
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        std::ofstream output;
        if (IBTK_MPI::getRank() == 0) output.open("output");
    ...
    // Last thing - collate the output into a single file named "output". The
    // file names and paths are hardcoded here (this is just a test after all)
    // but are available in the ConstraintIB input database.
    if (IBTK_MPI::getRank() == 0)
    {
        const std::vector<std::string> file_names = { "Cylinder_Drag_force_struct_no_0",
