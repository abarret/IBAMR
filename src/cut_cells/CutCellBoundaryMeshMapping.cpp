#include "ibamr/cut_cells/CutCellBoundaryMeshMapping.h"
...
namespace LS
{
CutCellBoundaryMeshMapping::CutCellBoundaryMeshMapping(std::string object_name,
                                                       Pointer<Database> input_db,
                                                       MeshBase* mesh,
                                                       FEDataManager* fe_data_manager)
    : CutCellMeshMapping(std::move(object_name), input_db), d_meshes({ mesh }), d_fe_data_managers({ fe_data_manager })
{
    d_bdry_id_to_skip_vec.resize(d_meshes.size());
    d_mapping_fcns.resize(d_meshes.size());
}
...
CutCellBoundaryMeshMapping::CutCellBoundaryMeshMapping(std::string object_name,
    ...
    : CutCellMeshMapping(std::move(object_name), input_db), d_meshes(meshes), d_fe_data_managers(fe_data_managers)
{
    d_bdry_id_to_skip_vec.resize(d_meshes.size());
    d_mapping_fcns.resize(d_meshes.size());
}

CutCellBoundaryMeshMapping::~CutCellBoundaryMeshMapping()
{
    if (d_is_initialized) deinitializeObjectState();
}

void
CutCellBoundaryMeshMapping::generateCutCellMappings()
{
...
bool
CutCellBoundaryMeshMapping::findIntersection(libMesh::Point& p,
                                             Elem* elem,
                                             libMesh::Point r,
