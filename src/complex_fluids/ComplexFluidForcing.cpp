...
void
ComplexFluidForcing::commonConstructor(const Pointer<Database> input_db,
                                       Pointer<VisItDataWriter<NDIM>> visit_data_writer,
                                       ...
                                       const std::vector<RobinBcCoefStrategy<NDIM>*> vel_bcs)
{
    ...
    d_sqr_root = input_db->getBoolWithDefault("square_root_evolve", d_sqr_root);
    d_log_conform = input_db->getBoolWithDefault("log_conform_evolve", d_log_conform);
                                         ...
                                         const bool initial_time)
{
    if (initial_time)
    {
        // TODO: The integrators do not call setDataOnPatchHierarchy for the initial iteration. This is a workaround for
        // this problem.
        if (!level->checkAllocated(d_W_scratch_idx)) level->allocatePatchData(d_W_scratch_idx);
        if (d_conform_draw && !level->checkAllocated(d_conform_idx_draw)) level->allocatePatchData(d_conform_idx_draw);
        if (d_stress_draw && !level->checkAllocated(d_stress_idx_draw)) level->allocatePatchData(d_stress_idx_draw);
...
void
ComplexFluidForcing::registerRelaxationOperator(Pointer<CFRelaxationOperator> rhs)
{
    d_convec_oper->registerRelaxationOperator(rhs);
    return;
}

void
ComplexFluidForcing::checkPositiveDefinite(const int data_idx,
