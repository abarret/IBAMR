...
public:
    ...
    static std::shared_ptr<PoissonSolver> allocate_solver(const std::string& object_name,
                                                                ...
                                                                const std::string& default_options_prefix)
    {
        std::shared_ptr<PoissonFACPreconditionerStrategy> fac_operator = std::make_shared<CCPoissonLevelRelaxationFACOperator>(
...
private:
    ...
    std::vector<std::shared_ptr<PoissonSolver> > d_level_solvers;
    std::shared_ptr<SAMRAI::tbox::Database> d_level_solver_db;
