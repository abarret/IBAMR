...
public:
     ...
     * data writer.
     */
    void registerCoordsData(std::shared_ptr<LData> coords_data, int level_number);

    /*!
     * \brief Register a variable for plotting with the Silo data writer.
     */
    void registerVariableData(const std::string& var_name, std::shared_ptr<LData> var_data, int level_number);
...
private:
     ...
     */
    std::vector<std::shared_ptr<LData> > d_coords_data;
    ...
    std::vector<std::vector<std::shared_ptr<LData> > > d_var_data;
