...
inline LNode::LNode(const int lagrangian_nidx,
                    ...
                    const SAMRAI::hier::IntVector& initial_periodic_offset,
                    const SAMRAI::hier::IntVector& current_periodic_offset,
                    const Vector& initial_periodic_displacement,
                    const Vector& current_periodic_displacement,
                    const std::vector<std::shared_ptr<Streamable> >& node_data)
...
inline void
LNode::registerPeriodicShift(const SAMRAI::hier::IntVector& offset, const Vector& displacement)
{
...
inline const std::vector<std::shared_ptr<Streamable> >&
...
inline void
LNode::setNodeData(const std::vector<std::shared_ptr<Streamable> >& node_data)
{
    d_node_data = node_data;
    setupNodeDataTypeArray();
    return;
...
inline void
LNode::appendNodeDataItem(const std::shared_ptr<Streamable>& node_data_item)
{
...
inline void
LNode::removeNodeDataItem(const std::shared_ptr<Streamable>& node_data_item)
{
    std::vector<std::shared_ptr<Streamable> >::iterator it =
...
inline void
LNode::copySourceItem(const SAMRAI::hier::Index& src_index,
...
inline void
LNode::packStream(SAMRAI::tbox::MessageStream& stream)
{
...
inline void
LNode::unpackStream(SAMRAI::tbox::MessageStream& stream, const SAMRAI::hier::IntVector& offset)
{
...
LNode::setupNodeDataTypeArray()
{
    ...
    Streamable* it_val;
    int class_id;
    for (std::vector<std::shared_ptr<Streamable> >::const_iterator cit = d_node_data.begin();
         cit != d_node_data.end();
         ++cit)
    {
        it_val = (*cit).get();
