...
public:
    ...
    LNode(int lagrangian_nidx = -1,
          ...
          const SAMRAI::hier::IntVector& initial_periodic_offset = SAMRAI::hier::IntVector::getZero(SAMRAI::tbox::Dimension(NDIM)),
          const SAMRAI::hier::IntVector& current_periodic_offset = SAMRAI::hier::IntVector::getZero(SAMRAI::tbox::Dimension(NDIM)),
          ...
          const std::vector<std::shared_ptr<Streamable> >& node_data =
              std::vector<std::shared_ptr<Streamable> >());
     ...
     */
    const std::vector<std::shared_ptr<Streamable> >& getNodeData() const;

    /*!
     * \brief Reset the collection of additional data items associated with the
     * node referenced by this LNode object.
     */
    void setNodeData(const std::vector<std::shared_ptr<Streamable> >& node_data);
     ...
     */
    void appendNodeDataItem(const std::shared_ptr<Streamable>& node_data_item);
     ...
     */
    void removeNodeDataItem(const std::shared_ptr<Streamable>& node_data_item);
     ...
     * boundary.
     */
    void registerPeriodicShift(const SAMRAI::hier::IntVector& offset, const Vector& displacement) override;
    ...
    virtual void unpackStream(SAMRAI::tbox::MessageStream& stream,
                              const SAMRAI::hier::IntVector& offset) override;
...
private:
    ...
    // the node
    std::vector<std::shared_ptr<Streamable> > d_node_data;
