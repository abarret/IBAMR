...
public:
     ...
     */
    virtual void setOperator(std::shared_ptr<GeneralOperator> op);

    /*!
     * \brief Retrieve the nonlinear operator \f$F[x]\f$ used by the solver.
     */
    virtual std::shared_ptr<GeneralOperator> getOperator() const;
     ...
     */
    virtual std::shared_ptr<SAMRAI::solv::SAMRAIVectorReal<double> > getSolutionVector() const = 0;
     ...
     * NULL pointer if the solver is not initialized.
     */
    virtual std::shared_ptr<SAMRAI::solv::SAMRAIVectorReal<double> > getFunctionVector() const = 0;
     ...
     * Jacobian.
     */
    virtual void setJacobian(std::shared_ptr<JacobianOperator> J);

    /*!
     * \brief Retrieve the Jacobian operator \f$J[x] = F'[x]\f$ used by the
     * solver.
     */
    virtual std::shared_ptr<JacobianOperator> getJacobian() const;

    /*!
     * \brief Retrieve the Krylov linear solver used in computing Newton step
     * directions.
     */
    virtual std::shared_ptr<KrylovLinearSolver> getLinearSolver() const;
...
protected:
    // Solver components.
    std::shared_ptr<GeneralOperator> d_F;
    std::shared_ptr<JacobianOperator> d_J;
    std::shared_ptr<KrylovLinearSolver> d_krylov_solver;
    std::shared_ptr<SAMRAI::solv::SAMRAIVectorReal<double> > d_x, d_b, d_r;
