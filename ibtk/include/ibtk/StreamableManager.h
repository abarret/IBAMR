...
public:
     ...
     */
    bool checkFactoryRegistration(std::shared_ptr<StreamableFactory> factory);
     ...
     * all MPI processes!
     */
    int registerFactory(std::shared_ptr<StreamableFactory> factory);

    /*!
     * \brief Return an upper bound on the amount of space required to pack a
     * Streamable object to a buffer.
     */
    size_t getDataStreamSize(std::shared_ptr<Streamable> data_item) const;

    /*!
     * \brief Return an upper bound on the amount of space required to pack a
     * vector of Streamable objects to a buffer.
     */
    size_t getDataStreamSize(const std::vector<std::shared_ptr<Streamable> >& data_items) const;

    /*!
     * \brief Pack a Streamable object into the output stream.
     */
    void packStream(SAMRAI::tbox::MessageStream& stream, std::shared_ptr<Streamable> data_item);

    /*!
     * \brief Pack a vector of Streamable objects into the output stream.
     */
    void packStream(SAMRAI::tbox::MessageStream& stream, std::vector<std::shared_ptr<Streamable> >& data_items);

    /*!
     * \brief Unpack a Streamable object from the data stream.
     */
    std::shared_ptr<Streamable> unpackStream(SAMRAI::tbox::MessageStream& stream,
...
private:
    using StreamableFactoryMap = std::map<int, std::shared_ptr<StreamableFactory> >;
