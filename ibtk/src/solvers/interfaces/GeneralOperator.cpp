...
void
GeneralOperator::setHierarchyMathOps(std::shared_ptr<HierarchyMathOps> hier_math_ops)
{
...
void
GeneralOperator::applyAdd(SAMRAIVectorReal<double>& x,
                          SAMRAIVectorReal<double>& y,
                          SAMRAIVectorReal<double>& z)
{
    // Guard against the case that y == z.
    std::shared_ptr<SAMRAIVectorReal<double> > zz = z.cloneVector(z.getName());
    zz->allocateVectorData();
    zz->copyVector(std::shared_ptr<SAMRAIVectorReal<double> >(&z, false));
    apply(x, *zz);
    z.add(std::shared_ptr<SAMRAIVectorReal<double> >(&y, false), zz);
    zz->freeVectorComponents();
    return;
} // applyAdd

void
GeneralOperator::initializeOperatorState(const SAMRAIVectorReal<double>& /*in*/,
...
void
GeneralOperator::modifyRhsForBcs(SAMRAIVectorReal<double>& /*y*/)
{
    // intentionally blank
    return;
} // modifyRhsForBcs
...
void
GeneralOperator::imposeSolBcs(SAMRAIVectorReal<double>& /*u*/)
{
    // intentionally blank
    return;
} // imposeSolBcs
