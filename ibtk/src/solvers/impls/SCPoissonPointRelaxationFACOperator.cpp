...
SCPoissonPointRelaxationFACOperator::SCPoissonPointRelaxationFACOperator(const std::string& object_name,
    ...
    : PoissonFACPreconditionerStrategy(
          object_name,
          std::make_shared<SideVariable<double>>(Dimension(NDIM), object_name + "::side_scratch", DEFAULT_DATA_DEPTH),
          SIDEG,
          input_db,
          std::move(default_options_prefix))
{
    ...
    std::shared_ptr<SideVariable<int> > mask_var = std::make_shared<SideVariable<NDIM, int>>(Dimension(NDIM), object_name + "::mask");
...
void
SCPoissonPointRelaxationFACOperator::smoothError(SAMRAIVectorReal<double>& error,
                                                 ...
                                                 bool /*performing_post_sweeps*/)
{
    ...
    if (level_num > d_coarsest_ln && num_sweeps > 1)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++, ++patch_counter)
        {
            ...
            std::shared_ptr<SideData<double> > error_data = error.getComponentPatchData(0, *patch);
            std::shared_ptr<SideData<double> > scratch_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(scratch_idx));
#if !defined(NDEBUG)
    ...
    for (int isweep = 0; isweep < num_sweeps; ++isweep)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++, ++patch_counter)
        {
            std::shared_ptr<Patch > patch = *p;
            std::shared_ptr<SideData<double> > error_data = error.getComponentPatchData(0, *patch);
            std::shared_ptr<SideData<double> > residual_data = residual.getComponentPatchData(0, *patch);
#if !defined(NDEBUG)
            const Box& ghost_box = error_data->getGhostBox();
...
#endif
            std::shared_ptr<SideData<int> > mask_data = std::static_pointer_cast<SideData<int> >(patch->getPatchData(d_mask_idx));
            ...
            if (update_local_data)
            {
                for (unsigned int axis = 0; axis < NDIM; ++axis)
                {
                    ...
                    for (const auto& pair : neighbor_overlap)
                    {
                        ...
                        const Box& overlap = pair.second;
                        std::shared_ptr<Patch > src_patch = level->getPatch(src_patch_num);
                        std::shared_ptr<SideData<double> > src_error_data = error.getComponentPatchData(0, *src_patch);
                        error_data->getArrayData(axis).copy(
                            src_error_data->getArrayData(axis), overlap, IntVector(Dimension(NDIM), 0));
                    }
                }
            }
...
bool
SCPoissonPointRelaxationFACOperator::solveCoarsestLevel(SAMRAIVectorReal<double>& error,
                                                        const SAMRAIVectorReal<double>& residual,
                                                        int coarsest_ln)
{
...
void
SCPoissonPointRelaxationFACOperator::computeResidual(SAMRAIVectorReal<double>& residual,
                                                     ...
                                                     int finest_level_num)
{
    ...
    const std::shared_ptr<SideVariable<double> > res_var = residual.getComponentVariable(0);
    const std::shared_ptr<SideVariable<double> > sol_var = solution.getComponentVariable(0);

    // Fill ghost-cell values.
    using InterpolationTransactionComponent = HierarchyGhostCellInterpolation::InterpolationTransactionComponent;
    std::shared_ptr<SideNoCornersFillPattern> fill_pattern = std::make_shared<SideNoCornersFillPattern>(d_gcw.max(), false, false, true);
                                                                        ...
                                                                        const int finest_reset_ln)
{
    ...
    std::shared_ptr<SideVariable<double> > solution_var = solution.getComponentVariable(0);
    std::shared_ptr<SideVariable<double> > rhs_var = rhs.getComponentVariable(0);

    std::shared_ptr<SideDataFactory<double> > solution_pdat_fac = solution_var->getPatchDataFactory();
    std::shared_ptr<SideDataFactory<double> > rhs_pdat_fac = rhs_var->getPatchDataFactory();
    ...
    VariableDatabase* var_db = VariableDatabase::getDatabase();
    std::shared_ptr<SideDataFactory<double> > scratch_pdat_fac =
    ...
    d_bc_helper = std::make_shared<StaggeredPhysicalBoundaryHelper>();
    ...
    d_cf_bdry_op = std::make_shared<CartSideDoubleQuadraticCFInterpolation>();
    ...
    d_bc_op = std::make_shared<CartSideRobinPhysBdryOp>(d_scratch_idx, d_bc_coefs, false);

    // Setup fill pattern spec objects.
    if (d_poisson_spec.dIsConstant())
    {
        d_op_stencil_fill_pattern = std::make_shared<SideNoCornersFillPattern>(d_gcw.max(), true, false, false);
    }
    else
    {
        d_op_stencil_fill_pattern = nullptr;
    }
    d_synch_fill_pattern = std::make_shared<SideSynchCopyFillPattern>();
    ...
    for (int ln = coarsest_reset_ln; ln <= finest_reset_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p1 = level->begin(); p1 != level->end(); p1++, ++patch_counter1)
        {
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                d_patch_neighbor_overlap[ln][patch_counter1][axis].clear();
            }
            std::shared_ptr<Patch > dst_patch = *p1;
            const Box& dst_patch_box = dst_patch->getBox();
            const Box& dst_ghost_box = Box::grow(dst_patch_box, 1);
            int patch_counter2 = 0;
            for (PatchLevel::Iterator p2 = level->begin(); patch_counter2 < patch_counter1 != level->end(); p2++, ++patch_counter2)
            {
                std::shared_ptr<Patch > src_patch = *p2;
