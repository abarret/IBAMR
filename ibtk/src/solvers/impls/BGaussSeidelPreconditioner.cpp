...
void
BGaussSeidelPreconditioner::setComponentPreconditioner(std::shared_ptr<LinearSolver> preconditioner,
...
void
BGaussSeidelPreconditioner::setComponentOperators(const std::vector<std::shared_ptr<LinearOperator> >& linear_ops,
                                                  const unsigned int component)
{
#if !defined(NDEBUG)
...
bool
BGaussSeidelPreconditioner::solveSystem(SAMRAIVectorReal<double>& x, SAMRAIVectorReal<NDIM, double>& b)
{
...
#if !defined(NDEBUG)
    std::shared_ptr<PatchHierarchy > hierarchy = x.getPatchHierarchy();
...
// Zero out the initial guess.
    ...
    // components.
    std::vector<std::shared_ptr<SAMRAIVectorReal<double> > > x_comps =
        getComponentVectors(std::shared_ptr<SAMRAIVectorReal<double> >(&x, false));
    std::vector<std::shared_ptr<SAMRAIVectorReal<double> > > b_comps =
        getComponentVectors(std::shared_ptr<SAMRAIVectorReal<double> >(&b, false));
    ...
    std::shared_ptr<SAMRAIVectorReal<double> > f = b.cloneVector(b.getName());
    f->allocateVectorData();
    f->copyVector(std::shared_ptr<SAMRAIVectorReal<double> >(&b, false), false);
    std::vector<std::shared_ptr<SAMRAIVectorReal<double> > > f_comps = getComponentVectors(f);
    ...
    for (auto it = comps.begin(); it != comps.end(); ++it, ++count)
    {
        ...
        std::shared_ptr<SAMRAIVectorReal<double> > x_comp = x_comps[comp];
        std::shared_ptr<SAMRAIVectorReal<double> > b_comp = b_comps[comp];
        std::shared_ptr<SAMRAIVectorReal<double> > f_comp = f_comps[comp];
        ...
        std::shared_ptr<LinearSolver> pc_comp = d_pc_map[comp];
...
void
BGaussSeidelPreconditioner::initializeSolverState(const SAMRAIVectorReal<double>& x,
                                                  const SAMRAIVectorReal<double>& b)
{
    ...
    // components.
    std::vector<std::shared_ptr<SAMRAIVectorReal<double> > > x_comps =
        getComponentVectors(Conststd::shared_ptr<SAMRAIVectorReal<double> >(&x, false));
    std::vector<std::shared_ptr<SAMRAIVectorReal<double> > > b_comps =
        getComponentVectors(Conststd::shared_ptr<SAMRAIVectorReal<double> >(&b, false));
...
BGaussSeidelPreconditioner::deallocateSolverState()
{
    ...
    {
        std::vector<std::shared_ptr<LinearOperator> >& comp_linear_ops = linearSolverVec_pair.second;
...
std::vector<std::shared_ptr<SAMRAIVectorReal<double> > >
BGaussSeidelPreconditioner::getComponentVectors(const Conststd::shared_ptr<SAMRAIVectorReal<double> > x)
{
    std::shared_ptr<PatchHierarchy > hierarchy = x->getPatchHierarchy();
    const int coarsest_ln = x->getCoarsestLevelNumber();
    const int finest_ln = x->getFinestLevelNumber();
    ...
    // Setup SAMRAIVectorReal objects to correspond to the individual vector
    // components.
    std::vector<std::shared_ptr<SAMRAIVectorReal<double> > > x_comps(ncomps);
    for (int comp = 0; comp < ncomps; ++comp)
    {
        x_comps[comp] = std::make_shared<SAMRAIVectorReal<double>>(
