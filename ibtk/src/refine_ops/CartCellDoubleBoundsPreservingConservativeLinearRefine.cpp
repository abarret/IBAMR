...
bool
CartCellDoubleBoundsPreservingConservativeLinearRefine::findRefineOperator(const std::shared_ptr<Variable >& var,
...
void
CartCellDoubleBoundsPreservingConservativeLinearRefine::refine(Patch& fine,
                                                               const Patch& coarse,
                                                               ...
                                                               const IntVector& ratio) const
{
    ...
    const hier::Index& patch_upper_crse = patch_box_crse.upper();
    ...
    for (int depth = 0; depth < data_depth; ++depth)
    {
        ...
        {
            const hier::Index& i_crse = b();
            const hier::Index i_fine = i_crse * ratio;
            ...
            for (Box::Iterator b(stencil_box_crse); b; b++)
            {
            ...
            stencil_box_fine.growUpper(ratio - IntVector(Dimension(NDIM), 1));
            double Delta = 0.0;
            for (Box::Iterator b(stencil_box_fine); b; b++)
            {
