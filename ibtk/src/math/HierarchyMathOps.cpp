...
HierarchyMathOps::HierarchyMathOps(std::string name,
    ...
    : d_object_name(std::move(name)),
      ...
      d_fc_var(new FaceVariable<double>(Dimension(NDIM), d_object_name + "::scratch_fc")),
      d_sc_var(new SideVariable<double>(Dimension(NDIM), d_object_name + "::scratch_sc")),
      d_of_var(new OuterfaceVariable<double>(d_object_name + "::scratch_of")),
      d_os_var(new OutersideVariable<double>(d_object_name + "::scratch_os")),
      d_coarsen_op_name(std::move(coarsen_op_name)),
      d_wgt_cc_var(new CellVariable<double>(Dimension(NDIM), d_object_name + "::wgt_cc", 1)),
      d_wgt_fc_var(new FaceVariable<double>(Dimension(NDIM), d_object_name + "::wgt_fc", 1)),
      d_wgt_sc_var(new SideVariable<double>(Dimension(NDIM), d_object_name + "::wgt_sc", 1))
{
...
void
HierarchyMathOps::setPatchHierarchy(std::shared_ptr<PatchHierarchy > hierarchy)
{
#if !defined(NDEBUG)
    ...
    std::shared_ptr<CellVariable<double> > cc_var = std::make_shared<CellVariable<NDIM, double>>(Dimension(NDIM), "cc_var");
    d_hier_cc_data_ops = hier_ops_manager->getOperationsDouble(cc_var, d_hierarchy, true);

    std::shared_ptr<FaceVariable<double> > fc_var = std::make_shared<FaceVariable<NDIM, double>>(Dimension(NDIM), "fc_var");
    d_hier_fc_data_ops = hier_ops_manager->getOperationsDouble(fc_var, d_hierarchy, true);

    std::shared_ptr<SideVariable<double> > sc_var = std::make_shared<SideVariable<NDIM, double>>(Dimension(NDIM), "sc_var");
...
HierarchyMathOps::resetLevels(const int coarsest_ln, const int finest_ln)
{
    ...
    {
        std::shared_ptr<PatchLevel > src_level = d_hierarchy->getPatchLevel(dst_ln + 1);
        std::shared_ptr<PatchLevel > dst_level = d_hierarchy->getPatchLevel(dst_ln);
...
std::shared_ptr<CellVariable<double> >
...
std::shared_ptr<FaceVariable<double> >
...
std::shared_ptr<SideVariable<double> >
...
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<CellVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<CellVariable<double> > src_var,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    ...
    if ((d_coarsest_ln == d_finest_ln) && (d_finest_ln == 0))
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<CellData<double> > src_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(src_idx));
    ...
    else
    {
        ...
        for (unsigned int d = 0; d < NDIM; ++d)
        {
            grad(d_sc_idx,
                 ...
                 0.0,
                 -1,
                 std::shared_ptr<SideVariable<double> >(nullptr),
                 d);
            ...
            for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
            {
                ...
                for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
                {
                    ...
                    std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
                    std::shared_ptr<SideData<double> > sc_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(d_sc_idx));
#if (NDIM == 2)
...
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<CellVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<FaceVariable<double> > /*src_var*/,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    if (src_ghost_fill) src_ghost_fill->fillData(src_ghost_fill_time);
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<FaceData<double> > src_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(src_idx));
...
void
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<FaceVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<FaceVariable<double> > /*src_var*/,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<FaceData<double> > dst_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<FaceData<double> > src_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(src_idx));
...
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<CellVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<SideVariable<double> > /*src_var*/,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    if (src_ghost_fill) src_ghost_fill->fillData(src_ghost_fill_time);
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<SideData<double> > src_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src_idx));
...
void
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<SideVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<SideVariable<double> > /*src_var*/,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > dst_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<SideData<double> > src_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src_idx));
...
void
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<NodeVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<SideVariable<double> > /*src_var*/,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<NodeData<double> > dst_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<SideData<double> > src_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src_idx));
...
void
HierarchyMathOps::curl(const int dst_idx,
                       const std::shared_ptr<EdgeVariable<double> > /*dst_var*/,
                       const int src_idx,
                       const std::shared_ptr<SideVariable<double> > /*src_var*/,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                       const double src_ghost_fill_time)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<EdgeData<double> > dst_data = std::static_pointer_cast<EdgeData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<SideData<double> > src_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src_idx));
...
HierarchyMathOps::rot(int dst_idx,
                      std::shared_ptr<SideVariable<double> > /*dst_var*/,
                      int src_idx,
                      std::shared_ptr<NodeVariable<double> > /*src_var*/,
                      std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                      ...
                      const std::vector<RobinBcCoefStrategy*>& bc_coefs)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > dst_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<NodeData<double> > src_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(src_idx));
...
HierarchyMathOps::rot(int dst_idx,
                      std::shared_ptr<SideVariable<double> > /*dst_var*/,
                      int src_idx,
                      std::shared_ptr<CellVariable<double> > /*src_var*/,
                      std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                      double src_ghost_fill_time,
                      const std::vector<RobinBcCoefStrategy*>& bc_coefs)
{
...
HierarchyMathOps::rot(int dst_idx,
                      std::shared_ptr<SideVariable<double> > /*dst_var*/,
                      int src_idx,
                      std::shared_ptr<EdgeVariable<double> > /*src_var*/,
                      std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                      double src_ghost_fill_time,
                      const std::vector<RobinBcCoefStrategy*>& bc_coefs)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > dst_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<EdgeData<double> > src_data = std::static_pointer_cast<EdgeData<double> >(patch->getPatchData(src_idx));
...
HierarchyMathOps::rot(int dst_idx,
                      std::shared_ptr<SideVariable<double> > /*dst_var*/,
                      int src_idx,
                      std::shared_ptr<SideVariable<double> > /*src_var*/,
                      std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                      double src_ghost_fill_time,
                      const std::vector<RobinBcCoefStrategy*>& bc_coefs)
{
...
HierarchyMathOps::div(const int dst_idx,
                      ...
                      const double alpha,
                      const int src1_idx,
                      const std::shared_ptr<CellVariable<double> > src1_var,
                      const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                      const double src1_ghost_fill_time,
                      const double beta,
                      const int src2_idx,
                      const std::shared_ptr<CellVariable<double> > src2_var,
                      const int dst_depth,
                      const int src2_depth)
{
    ...
    if ((d_coarsest_ln == d_finest_ln) && (d_finest_ln == 0))
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<CellData<double> > src1_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<CellData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
...
HierarchyMathOps::div(const int dst_idx,
                      ...
                      const double alpha,
                      const int src1_idx,
                      const std::shared_ptr<FaceVariable<double> > /*src1_var*/,
                      const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                      const double src1_ghost_fill_time,
                      ...
                      const int src2_depth)
{
    ...
    for (int ln = d_finest_ln; ln >= d_coarsest_ln; --ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<FaceData<double> > src1_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<CellData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            ...
            if ((ln > d_coarsest_ln) && src1_cf_bdry_synch)
            {
                std::shared_ptr<OuterfaceData<double> > of_data = std::static_pointer_cast<OuterfaceData<double> >(patch->getPatchData(d_of_idx));
...
HierarchyMathOps::div(const int dst_idx,
                      ...
                      const double alpha,
                      const int src1_idx,
                      const std::shared_ptr<SideVariable<double> > /*src1_var*/,
                      const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                      const double src1_ghost_fill_time,
                      const bool src1_cf_bdry_synch,
                      const double beta,
                      ...
                      const int src2_depth)
{
    ...
    for (int ln = d_finest_ln; ln >= d_coarsest_ln; --ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<SideData<double> > src1_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<CellData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            ...
            if ((ln > d_coarsest_ln) && src1_cf_bdry_synch)
            {
                std::shared_ptr<OutersideData<double> > os_data = std::static_pointer_cast<OutersideData<double> >(patch->getPatchData(d_os_idx));
                       ...
                       const int src1_depth)
{
    ...
    else
    {
        ...
        if (beta != 0.0)
        {
            ...
            const std::shared_ptr<CellVariable<double> > cc_var = dst_var;
...
HierarchyMathOps::grad(const int dst_idx,
                       ...
                       const double beta,
                       const int src2_idx,
                       const std::shared_ptr<FaceVariable<double> > /*src2_var*/,
                       const int src1_depth)
{
    if (src1_ghost_fill) src1_ghost_fill->fillData(src1_ghost_fill_time);
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<FaceData<double> > dst_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<CellData<double> > src1_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<FaceData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
...
HierarchyMathOps::grad(const int dst_idx,
                       ...
                       const double beta,
                       const int src2_idx,
                       const std::shared_ptr<SideVariable<double> > /*src2_var*/,
                       const int src1_depth)
{
    if (src1_ghost_fill) src1_ghost_fill->fillData(src1_ghost_fill_time);
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > dst_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<CellData<double> > src1_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<SideData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
...
HierarchyMathOps::grad(const int dst_idx,
                       const std::shared_ptr<CellVariable<double> > dst_var,
                       const int alpha_idx,
                       const std::shared_ptr<FaceVariable<double> > alpha_var,
                       const int src1_idx,
                       const std::shared_ptr<CellVariable<double> > src1_var,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                       const double src1_ghost_fill_time,
                       ...
                       const int src1_depth)
{
    ...
    grad(d_fc_idx,
         ...
         0.0,
         -1,
         std::shared_ptr<FaceVariable<double> >(nullptr),
         src1_depth);
...
HierarchyMathOps::grad(const int dst_idx,
                       const std::shared_ptr<CellVariable<double> > dst_var,
                       const int alpha_idx,
                       const std::shared_ptr<SideVariable<double> > alpha_var,
                       const int src1_idx,
                       const std::shared_ptr<CellVariable<double> > src1_var,
                       const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                       const double src1_ghost_fill_time,
                       ...
                       const int src1_depth)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<FaceData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<FaceData<double> > alpha_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(alpha_idx));
                       ...
                       const int src1_depth)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<SideData<double> > alpha_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(alpha_idx));
...
HierarchyMathOps::interp(const int dst_idx,
                         const std::shared_ptr<CellVariable<double> > /*dst_var*/,
                         const int src_idx,
                         const std::shared_ptr<NodeVariable<double> > /*src_var*/,
                         const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                         const double src_ghost_fill_time)
{
    if (src_ghost_fill) src_ghost_fill->fillData(src_ghost_fill_time);
...
HierarchyMathOps::interp(const int dst_idx,
                         const std::shared_ptr<CellVariable<double> > /*dst_var*/,
                         const int src_idx,
                         const std::shared_ptr<EdgeVariable<double> > /*src_var*/,
                         const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                         const double src_ghost_fill_time)
{
    if (src_ghost_fill) src_ghost_fill->fillData(src_ghost_fill_time);
                          ...
                          const int src2_depth)
{
    ...
    std::shared_ptr<SideVariable<double> > alpha_var;
    std::shared_ptr<CellVariable<double> > beta_var;

    bool nonaligned_anisotropy = false;
    if (!poisson_spec.dIsConstant())
    {
        VariableDatabase* var_db = VariableDatabase::getDatabase();
        std::shared_ptr<Variable > dummy_var;
        var_db->mapIndexToVariable(alpha_idx, dummy_var);
        alpha_var = dummy_var;
#if !defined(NDEBUG)
        TBOX_ASSERT(alpha_var);
#endif
        std::shared_ptr<SideDataFactory<double> > alpha_fac =
                          ...
                          const std::shared_ptr<SideVariable<double> > src2_var)
{
    ...
    std::shared_ptr<SideDataFactory<double> > dst_factory = dst_var->getPatchDataFactory();
    std::shared_ptr<SideDataFactory<double> > src1_factory = src1_var->getPatchDataFactory();
    ...
    if (src2_var)
    {
        std::shared_ptr<SideDataFactory<double> > src2_factory = src2_var->getPatchDataFactory();
...
HierarchyMathOps::vc_laplace(const int dst_idx,
                             ...
                             const double beta,
                             const int coef1_idx,
                             const std::shared_ptr<NodeVariable<double> > /*coef_var*/,
                             const int src1_idx,
                             const std::shared_ptr<SideVariable<double> > src1_var,
                             const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                             const double src1_ghost_fill_time,
                             ...
                             int coef2_idx,
                             std::shared_ptr<SideVariable<double> > /*coef2_var*/,
                             const double gamma,
                             const int src2_idx,
                             const std::shared_ptr<SideVariable<double> > src2_var)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > dst_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<NodeData<double> > coef1_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(coef1_idx));
            std::shared_ptr<SideData<double> > coef2_data =
                (coef2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(coef2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<SideData<double> > src1_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<SideData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
...
HierarchyMathOps::vc_laplace(const int dst_idx,
                             ...
                             const double beta,
                             const int coef1_idx,
                             const std::shared_ptr<EdgeVariable<double> > /*coef_var*/,
                             const int src1_idx,
                             const std::shared_ptr<SideVariable<double> > src1_var,
                             const std::shared_ptr<HierarchyGhostCellInterpolation> src1_ghost_fill,
                             const double src1_ghost_fill_time,
                             ...
                             const std::shared_ptr<SideVariable<double> > src2_var)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > dst_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<EdgeData<double> > coef1_data = std::static_pointer_cast<EdgeData<double> >(patch->getPatchData(coef1_idx));
            std::shared_ptr<SideData<double> > coef2_data =
                (coef2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(coef2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<SideData<double> > src1_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<SideData<double> > src2_data =
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    const std::shared_ptr<CellVariable<double> > /*dst_var*/,
                                    const int alpha_idx,
                                    const std::shared_ptr<CellVariable<double> > /*alpha_var*/,
                                    const int src1_idx,
                                    const std::shared_ptr<CellVariable<double> > /*src1_var*/,
                                    const double beta,
                                    const int src2_idx,
                                    ...
                                    const int alpha_depth)
{
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<CellData<double> > alpha_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(alpha_idx));
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    ...
                                    const std::shared_ptr<CellVariable<double> > /*src1_var*/,
                                    const int beta_idx,
                                    const std::shared_ptr<CellVariable<double> > /*beta_var*/,
                                    const int src2_idx,
                                    const std::shared_ptr<CellVariable<double> > /*src2_var*/,
                                    const int dst_depth,
                                    const int src1_depth,
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<CellData<double> > alpha_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(alpha_idx));
            std::shared_ptr<CellData<double> > beta_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(beta_idx));
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    ...
                                    const std::shared_ptr<FaceVariable<double> > /*src1_var*/,
                                    const int beta_idx,
                                    const std::shared_ptr<FaceVariable<double> > /*beta_var*/,
                                    const int src2_idx,
                                    const std::shared_ptr<FaceVariable<double> > /*src2_var*/,
                                    const int dst_depth,
                                    const int src1_depth,
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<FaceData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<FaceData<double> > alpha_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(alpha_idx));
            std::shared_ptr<FaceData<double> > beta_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(beta_idx));
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    ...
                                    const double alpha,
                                    const int src1_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*src1_var*/,
                                    const double beta,
                                    const int src2_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*src2_var*/,
                                    const int dst_depth,
                                    const int src1_depth,
                                    const int src2_depth)
{
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<NodeData<double> > dst_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(dst_idx));
            std::shared_ptr<NodeData<double> > src1_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(src1_idx));
            std::shared_ptr<NodeData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*dst_var*/,
                                    const int alpha_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*alpha_var*/,
                                    const int src1_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*src1_var*/,
                                    const double beta,
                                    const int src2_idx,
                                    ...
                                    const int alpha_depth)
{
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<NodeData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<NodeData<double> > alpha_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(alpha_idx));
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    ...
                                    const std::shared_ptr<NodeVariable<double> > /*src1_var*/,
                                    const int beta_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*beta_var*/,
                                    const int src2_idx,
                                    const std::shared_ptr<NodeVariable<double> > /*src2_var*/,
                                    const int dst_depth,
                                    const int src1_depth,
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<NodeData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<NodeData<double> > alpha_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(alpha_idx));
            std::shared_ptr<NodeData<double> > beta_data = std::static_pointer_cast<NodeData<double> >(patch->getPatchData(beta_idx));
...
HierarchyMathOps::pointwiseMultiply(const int dst_idx,
                                    ...
                                    const std::shared_ptr<SideVariable<double> > /*src1_var*/,
                                    const int beta_idx,
                                    const std::shared_ptr<SideVariable<double> > /*beta_var*/,
                                    const int src2_idx,
                                    const std::shared_ptr<SideVariable<double> > /*src2_var*/,
                                    const int dst_depth,
                                    const int src1_depth,
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<SideData<double> > src2_data =
                (src2_idx >= 0) ? std::static_pointer_cast<PatchData >(patch->getPatchData(src2_idx)) : std::shared_ptr<PatchData >();
            std::shared_ptr<SideData<double> > alpha_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(alpha_idx));
            std::shared_ptr<SideData<double> > beta_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(beta_idx));
...
HierarchyMathOps::strain_rate(const int dst1_idx,
                              const std::shared_ptr<CellVariable<double> > /*dst1_var*/,
                              const int dst2_idx,
                              const std::shared_ptr<CellVariable<double> > /*dst2_var*/,
                              const int src_idx,
                              const std::shared_ptr<SideVariable<double> > /*src_var*/,
                              const std::shared_ptr<HierarchyGhostCellInterpolation> src_ghost_fill,
                              const double src_ghost_fill_time)
{
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
            ...
            std::shared_ptr<CellData<double> > dst1_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst1_idx));
            std::shared_ptr<CellData<double> > dst2_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(dst2_idx));
            std::shared_ptr<SideData<double> > src_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(src_idx));
...
HierarchyMathOps::resetCoarsenOperators()
{
    ...
    d_of_coarsen_alg = std::make_shared<CoarsenAlgorithm>(Dimension(NDIM));
    d_of_coarsen_alg->registerCoarsen(d_fc_idx, // destination
                                      d_of_idx, // source
                                      d_of_coarsen_op);

    d_os_coarsen_alg = std::make_shared<CoarsenAlgorithm>(Dimension(NDIM));
...
#endif
    std::shared_ptr<CoarsenAlgorithm > coarsen_alg = std::make_shared<CoarsenAlgorithm>(Dimension(NDIM));
...
HierarchyMathOps::resetCellWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        if (ln < d_finest_ln)
        {
            std::shared_ptr<PatchLevel > next_finer_level = d_hierarchy->getPatchLevel(ln + 1);
...
#endif
                ;
            std::shared_ptr<CellData<double> > wgt_cc_data = std::static_pointer_cast<CellData<double> >(patch->getPatchData(d_wgt_cc_idx));
                    ...
                    for (unsigned int axis = 0; axis < NDIM; ++axis)
                    {
                        if (periodic_shift(axis) != 0)
                        {
                            for (int sgn = -1; sgn <= 1; sgn += 2)
                            {
                                ...
                                const Box refined_box =
                                    Box::shift(refined_region_boxes[i], periodic_offset);
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel::Iterator p = level->begin(); p != level->end(); p++)
        {
...
#endif
                ;
            std::shared_ptr<FaceData<double> > wgt_fc_data = std::static_pointer_cast<FaceData<double> >(patch->getPatchData(d_wgt_fc_idx));
            ...
            if (ln > d_coarsest_ln)
            {
                ...
                const std::vector<BoundaryBox >& cf_bdry_boxes = cf_bdry.getBoundaries(p(), bdry_type);
...
#endif
                ;
            std::shared_ptr<SideData<double> > wgt_sc_data = std::static_pointer_cast<SideData<double> >(patch->getPatchData(d_wgt_sc_idx));
