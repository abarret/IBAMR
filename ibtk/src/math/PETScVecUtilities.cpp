                                       ...
                                       std::shared_ptr<PatchLevel > patch_level)
{
    ...
    std::shared_ptr<Variable > data_var;
    var_db->mapIndexToVariable(data_idx, data_var);
    std::shared_ptr<CellVariable<double> > data_cc_var = data_var;
    std::shared_ptr<SideVariable<double> > data_sc_var = data_var;
    if (data_cc_var)
    {
#if !defined(NDEBUG)
...
PETScVecUtilities::copyFromPatchLevelVec(Vec& vec,
                                         ...
                                         const int dof_index_idx,
                                         std::shared_ptr<PatchLevel > patch_level,
                                         std::shared_ptr<RefineSchedule > data_synch_sched,
                                         std::shared_ptr<RefineSchedule > ghost_fill_sched)
{
    VariableDatabase* var_db = VariableDatabase::getDatabase();
    std::shared_ptr<Variable > data_var;
    var_db->mapIndexToVariable(data_idx, data_var);
    ...
    else if (data_sc_var)
    {
        ...
        if (data_synch_sched)
        {
            std::shared_ptr<RefineClasses > data_synch_config = data_synch_sched->getEquivalenceClasses();
            RefineAlgorithm data_synch_alg;
            data_synch_alg.registerRefine(data_idx, data_idx, data_idx, nullptr, std::make_shared<SideSynchCopyFillPattern>());
    ...
    if (ghost_fill_sched)
    {
        std::shared_ptr<RefineClasses > ghost_fill_config = ghost_fill_sched->getEquivalenceClasses();
...
PETScVecUtilities::constructDataSynchSchedule(const int data_idx, std::shared_ptr<PatchLevel > patch_level)
{
    VariableDatabase* var_db = VariableDatabase::getDatabase();
    std::shared_ptr<Variable > data_var;
    var_db->mapIndexToVariable(data_idx, data_var);
...
std::shared_ptr<RefineSchedule >
PETScVecUtilities::constructGhostFillSchedule(const int data_idx, std::shared_ptr<PatchLevel > patch_level)
{
    RefineAlgorithm ghost_fill_alg;
    ghost_fill_alg.registerRefine(data_idx, data_idx, data_idx, nullptr);
                                                      ...
                                                      std::shared_ptr<PatchLevel > patch_level)
{
    ...
    // "master" location.
    VariableDatabase* var_db = VariableDatabase::getDatabase();
    std::shared_ptr<SideVariable<int> > patch_num_var =
        std::make_shared<SideVariable<int>>(Dimension(NDIM), "PETScVecUtilities::constructPatchLevelDOFIndices_side()::patch_num_var");
    static const int patch_num_idx = var_db->registerPatchDataIndex(patch_num_var);
    patch_level->allocatePatchData(patch_num_idx);
    std::shared_ptr<SideVariable<bool> > mastr_loc_var =
        std::make_shared<SideVariable<bool>>(Dimension(NDIM), "PETScVecUtilities::constructPatchLevelDOFIndices_side()::mastr_loc_var");
    ...
    for (PatchLevel::Iterator p(patch_level); p != level->end(); p++)
    {
        ...
        std::shared_ptr<SideData<int> > dof_index_data = std::static_pointer_cast<SideData<int> >(patch->getPatchData(dof_index_idx));
        const int depth = dof_index_data->getDepth();
        std::shared_ptr<SideData<int> > patch_num_data = std::static_pointer_cast<SideData<int> >(patch->getPatchData(patch_num_idx));
        patch_num_data->fillAll(patch_num);
        std::shared_ptr<SideData<bool> > mastr_loc_data = std::static_pointer_cast<SideData<bool> >(patch->getPatchData(mastr_loc_idx));
    ...
    bdry_synch_alg.registerRefine(patch_num_idx, patch_num_idx, patch_num_idx, nullptr, std::make_shared<SideSynchCopyFillPattern>());
    bdry_synch_alg.registerRefine(dof_index_idx, dof_index_idx, dof_index_idx, nullptr, std::make_shared<SideSynchCopyFillPattern>());
    ...
    dof_synch_alg.registerRefine(dof_index_idx, dof_index_idx, dof_index_idx, nullptr, std::make_shared<SideSynchCopyFillPattern>());
                                              ...
                                              const int ao_offset)
{
    ...
    const hier::Index& domain_upper = domain_boxes[0].upper();
    std::shared_ptr<CartesianGridGeometry > grid_geom = patch_level->getGridGeometry();
    IntVector periodic_shift = grid_geom->getPeriodicShift(patch_level->getRatio());
    hier::Index num_cells = 1;
                                              ...
                                              const int ao_offset)
{
    ...
    std::shared_ptr<CartesianGridGeometry > grid_geom = patch_level->getGridGeometry();
    IntVector periodic_shift = grid_geom->getPeriodicShift(patch_level->getRatio());
    std::array<hier::Index, NDIM> num_cells;
    ...
    for (PatchLevel::Iterator p(patch_level); p != level->end(); p++)
    {
        ...
        for (unsigned int component_axis = 0; component_axis < NDIM; ++component_axis)
        {
            ...
            {
                const CellIndex& i = b();
                const SideIndex is(i, component_axis, SideIndex::Lower);
