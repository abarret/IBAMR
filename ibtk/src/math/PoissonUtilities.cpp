...
{
inline Box
compute_tangential_extension(const Box& box, const int data_axis)
{
...
#if (NDIM == 2)
inline double
compute_mu_avg(const hier::Index& i, const NodeData<double>& mu_data)
{
    Box node_box(i, i);
    ...
    double avg_mu = 0.0;
    int total_nodes = 0;
    NodeIterator end(pdat::NodeGeometry::end(node_box));
    for (NodeIterator n(pdat::NodeGeometry::begin(node_box)); n != end; n++, total_nodes++)
    {
...
inline double
compute_mu_harmonic_avg(const hier::Index& i, const NodeData<double>& mu_data)
{
    Box node_box(i, i);
    const int n_nodes = std::pow(2, NDIM);
...
#if (NDIM == 3)
inline double
compute_mu_avg(const hier::Index& i, const EdgeData<double>& mu_data)
{
    Box edge_box(i, i);
    ...
    {
        for (EdgeIterator e(edge_box, axis); e; e++, total_edges++)
        {
...
inline double
compute_mu_harmonic_avg(const hier::Index& i, const EdgeData<double>& mu_data)
{
    Box edge_box(i, i);
    const int n_edges = 12;
...
inline double
get_mu_edge(const hier::Index& i, const int perp, const std::shared_ptr<EdgeData<double> > mu_data)
{
    const ArrayData<double>& mu_array_data = mu_data->getArrayData(perp);
...
void
PoissonUtilities::computeMatrixCoefficients(CellData<double>& matrix_coefficients,
                                            std::shared_ptr<Patch > patch,
                                            const std::vector<hier::Index >& stencil,
                                            const PoissonSpecifications& poisson_spec,
                                            RobinBcCoefStrategy* bc_coef,
                                            double data_time)
{
    std::vector<RobinBcCoefStrategy*> bc_coefs(1, bc_coef);
                                            ...
                                            double data_time)
{
...
#endif
    std::map<hier::Index, int, IndexFortranOrder> stencil_map;
    for (int k = 0; k < stencil_sz; ++k)
    {
        stencil_map[stencil[k]] = k;
    }
#if !defined(NDEBUG)
    TBOX_ASSERT(stencil_map.find(hier::Index(0)) != stencil_map.end());
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        hier::Index ilower(0), iupper(0);
        ilower(axis) = -1;
        iupper(axis) = +1;
...
#endif
    const int stencil_index_diag = stencil_map[hier::Index(0)];
    ...
    CellData<double> diagonal(patch_box, depth, IntVector(Dimension(NDIM), 0));
    SideData<double> off_diagonal(patch_box, depth, IntVector(Dimension(NDIM), 0));
    ...
    for (int n = 0; n < n_physical_codim1_boxes; ++n)
    {
        ...
        std::shared_ptr<ArrayData<double> > acoef_data = std::make_shared<ArrayData<NDIM, double>>(bc_coef_box, 1);
        std::shared_ptr<ArrayData<double> > bcoef_data = std::make_shared<ArrayData<NDIM, double>>(bc_coef_box, 1);
        std::shared_ptr<ArrayData<double> > gcoef_data = std::make_shared<ArrayData<NDIM, double>>(bc_coef_box, 1);
...
void
PoissonUtilities::computeMatrixCoefficients(SideData<double>& matrix_coefficients,
                                            std::shared_ptr<Patch > patch,
                                            const std::vector<hier::Index >& stencil,
                                            const PoissonSpecifications& poisson_spec,
                                            const std::vector<RobinBcCoefStrategy*>& bc_coefs,
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            const Box bc_fill_box =
                pgeom->getBoundaryFillBox(bdry_box, patch_box, /* ghost_width_to_fill */ IntVector(Dimension(NDIM), 1));
...
PoissonUtilities::computeVCSCViscousOpMatrixCoefficients(
    ...
    const std::vector<std::map<hier::Index, int, IndexFortranOrder> >& stencil_map_vec,
    ...
    VCInterpType mu_interp_type)
{
    ...
    std::shared_ptr<SideData<double> > C_data = nullptr;
    if (C_is_varying) C_data = patch->getPatchData(poisson_spec.getCPatchDataId());

#if (NDIM == 2)
    std::shared_ptr<NodeData<double> > mu_data = patch->getPatchData(poisson_spec.getDPatchDataId());
#elif (NDIM == 3)
    std::shared_ptr<EdgeData<double> > mu_data = patch->getPatchData(poisson_spec.getDPatchDataId());
#endif
...
#if (NDIM == 2)
    const ArrayData<double>& mu_array_data = mu_data->getArrayData();
#endif
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        ...
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            for (Box::Iterator bc(bc_coef_box); bc; bc++)
            {
                ...
                if (is_lower)
                {
                    hier::Index shift = get_shift(bdry_normal_axis, -1);
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        ...
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            const BoundaryBox trimmed_bdry_box =
                PhysicalBoundaryUtilities::trimBoundaryCodim1Box(bdry_box, *patch);
            const Box side_box = PhysicalBoundaryUtilities::makeSideBoundaryCodim1Box(trimmed_bdry_box);

            for (unsigned int comp = 0; comp < NDIM; ++comp)
            {
                if (comp == axis) continue;

                const Box bc_coef_box = compute_tangential_extension(side_box, comp);
                ...
                //
                //     u_o = -((a*h - 2*b)/(a*h + 2*b))*u_i
                for (Box::Iterator bc(side_box); bc; bc++)
                {
                    const hier::Index& i = bc();
                    const hier::Index i_upper = i + get_shift(comp, 1);
                    ...
                    if (is_lower)
                    {
                        hier::Index shift_outer_lower = get_shift(bdry_normal_axis, -1) + get_shift(comp, -1);
                        hier::Index shift_inner_lower = get_shift(bdry_normal_axis, 1) + get_shift(comp, -1);
                        ...
                        hier::Index shift_outer_upper = get_shift(bdry_normal_axis, -1) + get_shift(comp, 1);
                        hier::Index shift_inner_upper = get_shift(bdry_normal_axis, 1) + get_shift(comp, 1);
        ...
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            for (Box::Iterator bc(bc_coef_box); bc; bc++)
            {
                ...
                else
                {
                    ...
                    if (is_lower)
                    {
                        const hier::Index shift_outer = get_shift(bdry_normal_axis, -1);
                        const hier::Index shift_inner = get_shift(bdry_normal_axis, 1);
                        matrix_coefficients(i_s, stencil_map[ORIGIN]) -=
...
void
PoissonUtilities::adjustRHSAtPhysicalBoundary(CellData<double>& rhs_data,
                                              std::shared_ptr<Patch > patch,
                                              const PoissonSpecifications& poisson_spec,
                                              RobinBcCoefStrategy* bc_coef,
                                              double data_time,
...
void
PoissonUtilities::adjustRHSAtPhysicalBoundary(SideData<double>& rhs_data,
                                              std::shared_ptr<Patch > patch,
                                              const PoissonSpecifications& poisson_spec,
                                              const std::vector<RobinBcCoefStrategy*>& bc_coefs,
                                              double data_time,
...
void
PoissonUtilities::adjustVCSCViscousOpRHSAtPhysicalBoundary(SideData<double>& rhs_data,
                                                           std::shared_ptr<Patch > patch,
                                                           const PoissonSpecifications& poisson_spec,
                                                           double alpha,
                                                           const std::vector<RobinBcCoefStrategy*>& bc_coefs,
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            for (Box::Iterator bc(bc_coef_box); bc; bc++)
            {
                ...
                const hier::Index shift_bdry_normal = get_shift(bdry_normal_axis, 1);
#if (NDIM == 2)
        ...
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            for (unsigned int comp = 0; comp < NDIM; ++comp)
            {
                ...
                for (Box::Iterator bc(side_box); bc; bc++)
                {
                    ...
                    const hier::Index shift_d = get_shift(comp, 1);
#if (NDIM == 2)
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        for (int n = 0; n < n_physical_codim1_boxes; ++n)
        {
            ...
            for (Box::Iterator bc(bc_coef_box); bc; bc++)
            {
                ...
                const hier::Index shift_axis_minus = get_shift(bdry_normal_axis, -1);
                double mu_upper = std::numeric_limits<double>::quiet_NaN();
                double mu_lower = std::numeric_limits<double>::quiet_NaN();
                if (mu_interp_type == VC_AVERAGE_INTERP)
                {
...
void
PoissonUtilities::adjustRHSAtCoarseFineBoundary(CellData<double>& rhs_data,
                                                const CellData<double>& sol_data,
                                                std::shared_ptr<Patch > patch,
                                                const PoissonSpecifications& poisson_spec,
                                                const std::vector<BoundaryBox >& type1_cf_bdry)
{
    const int depth = rhs_data.getDepth();
    ...
    const IntVector ghost_width_to_fill(1);
    for (int n = 0; n < n_bdry_boxes; ++n)
    {
        const BoundaryBox& bdry_box = type1_cf_bdry[n];
        const BoundaryBox trimmed_bdry_box = PhysicalBoundaryUtilities::trimBoundaryCodim1Box(bdry_box, *patch);
        const Box bc_fill_box = pgeom->getBoundaryFillBox(trimmed_bdry_box, patch_box, ghost_width_to_fill);
        const unsigned int location_index = bdry_box.getLocationIndex();
        const unsigned int bdry_normal_axis = location_index / 2;
        const double h = dx[bdry_normal_axis];
        ...
        for (int d = 0; d < depth; ++d)
        {
            for (Box::Iterator bc(bc_fill_box); bc; bc++)
            {
                hier::Index i_s_bdry = bc();
                CellIndex i_c_intr(i_s_bdry);
                CellIndex i_c_bdry(i_s_bdry);
                if (is_lower)
                {
                    i_c_intr(bdry_normal_axis) += 1;
...
void
PoissonUtilities::adjustRHSAtCoarseFineBoundary(SideData<double>& rhs_data,
                                                const SideData<double>& sol_data,
                                                std::shared_ptr<Patch > patch,
                                                const PoissonSpecifications& poisson_spec,
                                                const std::vector<BoundaryBox >& type1_cf_bdry)
{
    ...
    for (int n = 0; n < n_bdry_boxes; ++n)
    {
        ...
        for (unsigned int axis = 0; axis < NDIM; ++axis)
        {
            const Box bc_fill_box_axis =
                bdry_normal_axis == axis ? bc_fill_box : compute_tangential_extension(bc_fill_box, axis);
            for (int d = 0; d < depth; ++d)
            {
                for (Box::Iterator bc(bc_fill_box); bc; bc++)
                {
                    SideIndex i_s_intr(bc(), axis, SideIndex::Lower);
                    SideIndex i_s_bdry(bc(), axis, SideIndex::Lower);
                    if (is_lower)
                    {
...
void
PoissonUtilities::adjustVCSCViscousOpRHSAtCoarseFineBoundary(SideData<double>& rhs_data,
                                                             const SideData<double>& sol_data,
                                                             std::shared_ptr<Patch > patch,
                                                             const PoissonSpecifications& poisson_spec,
                                                             double alpha,
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        for (int n = 0; n < n_cf_bdry_boxes; ++n)
        {
            ...
            for (Box::Iterator bc(bc_coef_box); bc; bc++)
            {
                ...
                const SideIndex i_s_intr(i_intr, axis, SideIndex::Lower);
                const SideIndex i_s_bdry(i_bdry, axis, SideIndex::Lower);
        ...
        for (int n = 0; n < n_cf_bdry_boxes; ++n)
        {
            ...
            for (unsigned int comp = 0; comp < NDIM; ++comp)
            {
                ...
                for (Box::Iterator bc(side_box); bc; bc++)
                {
                    ...
                    {
                        const hier::Index shift_axis_minus = get_shift(axis, -1);
                        const SideIndex sw(i + shift_axis_minus, comp, SideIndex::Lower);
                        const SideIndex nw(i + shift_axis_minus, comp, SideIndex::Upper);
                    ...
                    {
                        const hier::Index shift_axis_plus = get_shift(axis, 1);
                        const SideIndex se(i, comp, SideIndex::Lower);
                        const SideIndex ne(i, comp, SideIndex::Upper);
    ...
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        for (int n = 0; n < n_cf_bdry_boxes; ++n)
        {
            ...
            for (Box::Iterator bc(bc_coef_box); bc; bc++)
            {
                ...
                const hier::Index shift_axis_minus = get_shift(bdry_normal_axis, -1);
                const hier::Index shift_axis_plus = get_shift(bdry_normal_axis, 1);

                const SideIndex i_s(i, bdry_normal_axis, SideIndex::Lower);
                const SideIndex i_s_bdry(
                    i + (is_lower ? shift_axis_minus : shift_axis_plus), bdry_normal_axis, SideIndex::Lower);
