...
EdgeSynchCopyFillPattern::calculateOverlap(const BoxGeometry& dst_geometry,
...
{
    std::shared_ptr<EdgeOverlap > box_geom_overlap =
    ...
    auto const t_dst_geometry = dynamic_cast<const EdgeGeometry*>(&dst_geometry);
#if !defined(NDEBUG)
    ...
    return std::make_shared<EdgeOverlap>(dst_boxes, src_offset);
} // calculateOverlap
