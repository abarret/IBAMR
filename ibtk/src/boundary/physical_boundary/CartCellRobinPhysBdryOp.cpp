...
void
CartCellRobinPhysBdryOp::setPhysicalBoundaryConditions(Patch& patch,
...
IntVector
CartCellRobinPhysBdryOp::getRefineOpStencilWidth(const Dimension& dim) const
{
    return IntVector(dim, REFINE_OP_STENCIL_WIDTH);
} // getRefineOpStencilWidth

void
CartCellRobinPhysBdryOp::accumulateFromPhysicalBoundaryData(Patch& patch,
                                                            const double fill_time,
                                                            const IntVector& ghost_width_to_fill)
{
    if (ghost_width_to_fill == IntVector(Dimension(NDIM), 0)) return;
                                                   ...
                                                   const bool adjoint_op)
{
    ...
    for (int n = 0; n < n_physical_codim1_boxes; ++n)
    {
        ...
        std::shared_ptr<ArrayData<double> > acoef_data = std::make_shared<ArrayData<double>>(bc_coef_box, 1);
        std::shared_ptr<ArrayData<double> > bcoef_data = std::make_shared<ArrayData<double>>(bc_coef_box, 1);
        std::shared_ptr<ArrayData<double> > gcoef_data = std::make_shared<ArrayData<double>>(bc_coef_box, 1);
        for (int d = 0; d < patch_data_depth; ++d)
        {
            RobinBcCoefStrategy* bc_coef = d_bc_coefs[d];
