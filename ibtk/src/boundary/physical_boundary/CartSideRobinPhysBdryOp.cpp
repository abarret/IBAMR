...
compute_tangential_extension(const Box& box, const int data_axis)
{
    ...
    extended_box.growUpper(data_axis, 1);
    return extended_box;
...
void
CartSideRobinPhysBdryOp::setPhysicalBoundaryConditions(Patch& patch,
...
IntVector
CartSideRobinPhysBdryOp::getRefineOpStencilWidth(const SAMRAI::tbox::Dimension& dim) const
{
    return IntVector(dim, REFINE_OP_STENCIL_WIDTH);
} // getRefineOpStencilWidth

void
CartSideRobinPhysBdryOp::accumulateFromPhysicalBoundaryData(Patch& patch,
                                                            const double fill_time,
                                                            const IntVector& ghost_width_to_fill)
{
    if (ghost_width_to_fill == IntVector(Dimension(NDIM), 0)) return;
                                                         ...
                                                         const bool adjoint_op)
{
    ...
    for (int n = 0; n < n_physical_codim1_boxes; ++n)
    {
        ...
        auto gcoef_data = std::make_shared<ArrayData<double>>(bc_coef_box, 1);
                                                             ...
                                                             const bool adjoint_op)
{
    ...
    std::vector<Box> side_box(NDIM, Box(Dimension(NDIM)));
    ...
    PatchGeometry::TwoDimBool touches_regular_bdry(Dimension(NDIM));
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
    ...
    for (int n = 0; n < n_physical_codim1_boxes; ++n)
    {
        ...
        for (unsigned int axis = 0; axis < NDIM; ++axis)
        {
            if (axis != bdry_normal_axis)
            {
                ...
                patch.setPatchGeometry(std::make_shared<CartesianPatchGeometry>(ratio_to_level_zero,
