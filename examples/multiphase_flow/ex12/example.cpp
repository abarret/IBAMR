...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        if (!is_from_restart)
        {
            ...
            wedge.X0(1) = input_db->getDouble("WEDGE_Y");
#if (NDIM == 3)
            wedge.X0(2) = input_db->getDouble("WEDGE_Z");
#endif
        }
        else
        {
            const std::vector<std::vector<double> >& structure_COM = ib_method_ops->getCurrentStructureCOM();
            wedge.X0(0) = structure_COM[0][0];
            wedge.X0(1) = structure_COM[0][1];
#if (NDIM == 3)
            wedge.X0(2) = structure_COM[0][2];
#endif
        }
        wedge.wedge_locate_method = input_db->getStringWithDefault("WEDGE_LOCATE_METHOD", "GEOMETRY_METHOD");
        ...
        // struct_0
        const string& object_name = "Wedge";
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            // Update the position of wedge reference point.
            const std::vector<double>& vel_COM = ib_method_ops->getCurrentCOMVelocity()[0];
            wedge.X0(NDIM - 1) += vel_COM[NDIM - 1] * dt;
