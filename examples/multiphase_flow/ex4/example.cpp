...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        LSLocateLayerInterface* ptr_LSLocateLayerInterface =
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            const double dP_max = hier_p_data_ops.max(p_idx) - hier_p_data_ops.min(p_idx);
            pout << std::setprecision(16) << "Pressure difference = " << dP_max << std::endl;
            pout << std::setprecision(16) << "Absolute dP error = " << std::abs(dP_max - dP_exact) << std::endl;
            pout << std::setprecision(16) << "Relative dP error = " << std::abs(dP_max - dP_exact) / dP_exact
                 << std::endl;
            pout << std::setprecision(16) << "Max norm velocity = " << hier_sc_data_ops.maxNorm(u_idx, wgt_sc_idx);
        ...
        delete ptr_LSLocateLayerInterface;
