                                                   ...
                                                   const NodeData<NDIM, double>& ls_new_data)
{
    ...
    if (vol_new_data(idx) < 1.0 && vol_new_data(idx) > 0.0)
    {
        ...
        {
            intersect_pts.push_back(
                LS::midpoint_value(VectorNd(idx(0) + 1.0, idx(1)), ls_lr, VectorNd(idx(0) + 1.0, idx(1) + 1.0), ls_ur));
        }
        if (ls_ur * ls_ul < 0.0)
        {
            intersect_pts.push_back(
                LS::midpoint_value(VectorNd(idx(0) + 1.0, idx(1) + 1.0), ls_ur, VectorNd(idx(0), idx(1) + 1.0), ls_ul));
        }
        if (ls_ul * ls_ll < 0.0)
        {
            intersect_pts.push_back(
                                   ...
                                   double sgn)
{
    ...
    // This point is inside the domain. If normal_vec contains NaNs, then we return true
    for (int d = 0; d < NDIM; ++d)
    {
        if (normal_vec(d) != normal_vec(d)) return true;
    }
    // We are on a cut cell. Find the cell centroid, then make sure the dot product is the same sign as sgn.
