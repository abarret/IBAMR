...
{
    //    bool print = d_time > 0.0299;
    //    const int coarsest_ln = 0;
    //    const int finest_ln = d_hierarchy->getFinestLevelNumber();
    //    // Free any preallocated matrices
    //    for (std::vector<std::map<IndexList, FullPivHouseholderQR<IBTK::MatrixXd> > >& qr_matrix_map :
    //    d_qr_matrix_vec)
    //        qr_matrix_map.clear();
    //    for (std::map<IndexList, std::vector<hier::Index<NDIM> > >& idx_map : d_reconstruct_idxs_map_vec)
    //    idx_map.clear();
    //
    //
    //    if (print) plog << "allocating storage\n";
    ...
    //        std::vector<std::map<IndexList, FullPivHouseholderQR<IBTK::MatrixXd> > >& qr_map_vec =
    //        d_qr_matrix_vec[ln]; qr_map_vec.resize(level->getNumberOfPatches()); std::map<IndexList,
    //        std::vector<hier::Index<NDIM> > >& idx_map = d_reconstruct_idxs_map_vec[ln]; int local_patch_num = 0; for
    //        (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num)
    //        {
    ...
    //                    if (print) plog << "Caching data for index " << idx << "\n";
    //                    if (print) plog << "physical location: " << x_loc.transpose() << "\n";
    //                    Box<NDIM> box(idx, idx);
    //                    box.grow(d_stencil_size);
    //                    if (print) plog << "searching on box " << box << "\n";
    //#ifndef NDEBUG
    ...
    //                            LS::midpoint_value(VectorNd(idx(0), idx(1)), ls_ll, VectorNd(idx(0) + 1.0, idx(1)),
    //                            ls_lr));
    //                    }
    //                    if (ls_lr * ls_ur < 0.0)
    //                    {
    ...
    //                    {
    //                        intersect_pts.push_back(
    //                            LS::midpoint_value(VectorNd(idx(0), idx(1) + 1.0), ls_ul, VectorNd(idx(0), idx(1)),
    //                            ls_ll));
    //                    }
    //                    if (print) plog << "num of intersection points" << intersect_pts.size() << "\n";
    //                    IBTK::Vector3d tangent_vec(
    //                        intersect_pts[0](0) - intersect_pts[1](0), intersect_pts[0](1) - intersect_pts[1](1),
    //                        0.0);
    ...
    //                                LS::find_cell_centroid(idx_c, *ls_data) - 0.5 * (intersect_pts[0] +
    //                                intersect_pts[1]);
    //
                                          ...
                                          Pointer<Patch<NDIM> > patch)
{
    //    bool print = d_time > 0.0299;
    bool print = false;
    if (print) plog << "updating cache\n";
    if (d_update_weights) cacheData();
    if (print) plog << "finished updating cache\n";
    ...
    if (print)
    {
        plog << "We have found " << X_vals.size() << " points.\n";
        plog << "Searching for a QR decomposition.\n";
        plog << "There are: "
             << d_qr_matrix_vec[ln][patch->getPatchNumber()][IndexList(patch, idx)].matrixQ().rows() - (NDIM + 1)
             << " in the base.\n";
        plog << "Found a QR decomposition.\n";
    }
                                 ...
                                 Pointer<Patch<NDIM> > patch)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(level); p; p++, ++local_patch_num)
    {
        ...
        if ((*vol_data)(idx) < 1.0 && (*vol_data)(idx) > 0.0)
        {
            ...
            TBOX_ASSERT(vol_data->getGhostBox().contains(box));
#endif
            if (print) plog << "idx " << idx << "\n";
            if (print) plog << "xloc: " << x_loc.transpose() << "\n";
            if (print) plog << "box: " << box << "\n";
            // Find tangent line
            std::vector<VectorNd> intersect_pts;
            double ls_ll = (*ls_data)(NodeIndex<NDIM>(idx, NodeIndex<NDIM>::LowerLeft));
            double ls_lr = (*ls_data)(NodeIndex<NDIM>(idx, NodeIndex<NDIM>::LowerRight));
            ...
            if (print) plog << "Num of intersections " << intersect_pts.size() << "\n";
            if (print) plog << "Volume of cell: " << (*vol_data)(idx) << "\n";
            if (print)
            {
                plog << "ls_ll: " << ls_ll << "\n";
                plog << "ls_lr: " << ls_lr << "\n";
            ...
            // We know that the current idx is on the "interior" of the structure.
            // Use this to determine the sign
            VectorNd cur_vec = LS::find_cell_centroid(idx, *ls_data) - 0.5 * (intersect_pts[0] + intersect_pts[1]);
            double sgn = normal_vec_2d.dot(cur_vec);
            // Now we need to check against this sign to determine if we can use the point.
        ...
        else
        {
            ...
            // If we are on an empty cell, it's because
            if (print) plog << "Not a cut cell\n";
            if (print) plog << "Vol: " << (*vol_data)(idx) << "\n";
