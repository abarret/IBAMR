...
ExtraStressExact::ExtraStressExact(const string object_name, Pointer<Database> input_db, const int depth)
    : CartGridFunction(object_name), d_depth(depth)
{
                                 ...
                                 Pointer<PatchLevel<NDIM> > /*level*/)
{
    ...
    for (CellIterator<NDIM> ci(patch_box); ci; ci++)
    {
        ...
        {
            sig(0, 0) = 2.0 * std::pow(d_la * d_dpdx * x_ref[1] / (d_mu + d_mup), 2.0) + 1.0;
            sig(1, 1) = 1.0;
            sig(0, 1) = sig(1, 0) = d_la * d_dpdx * x_ref[1] / (d_mu + d_mup);
        }
...
{
    std::pair<int, int> idx = voigt_to_tensor_idx(d_depth);
    ...
    for (Box<NDIM>::Iterator bc(box); bc; bc++)
    {
        ...
        if (axis == 0)
        {
            ...
            // Now transform back to original configuration
            sig = d_Q * sig * d_Q.transpose();
            if (!gcoef_data.isNull()) (*gcoef_data)(i, 0) = sig(idx.first, idx.second);
        }
        else if (axis == 1)
        {
